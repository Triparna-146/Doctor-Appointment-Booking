  ...
  return userData && (
    <div className="max-w-lg flex flex-col gap-2 text-sm">

      {
        ...
        : <img className="w-36 ronded" src={userData.image} alt="" /> 
      }

      {isEdit ? (
        <input className="bg-gray-50 text-3xl font-medium text-neutral-600 max-w-60 mt-4"
          type="text"
          value={userData.name}
          onChange={(e) =>
      ...
      <div>
        ...
        <div className="grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700">
          ...
          {isEdit ? (
            <input className="bg-gray-100 max-w-52 outline-none"
              type="text"
          ...
          {isEdit ? (
            <p className="max-w-52">
              <input className="bg-gray-100 outline-none w-52"
                type="text"
      ...
      <div>
        ...
        <div className="grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700">
          ...
          {
            isEdit
            ? <select className="max-w-28 bg-gray-100 outline-none" onChange={(e) => setUserData(prev => ({...prev, gender:e.target.value}))} value={userData.gender}>
              <option value="Male">Male</option>
              <option value="Female">Female</option>
            </select>
            : <p className="text-neutral-500">{userData.gender}</p>
          }

          <p>Birthday:</p>
          {
            isEdit
            ? <input className="max-w-28 bg-gray-100 outline-none" type="date" onChange={(e) => setUserData(prev => ({...prev, dob: e.target.value === "Not Selected" ? "" : e.target.value}))} value={userData.dob} />
            : <p className="text-neutral-500">{userData.dob}</p>
          }
